...
export class CommandHandler {
    ...
    async deploy(fullRoute) {
        ...
        try {
            console.log(`Started refreshing ${commands.each.length} application (/) commands.`);
            const data = await rest.put(fullRoute, {
                body: commands.keys,
            });
            console.log(`Successfully reloaded ${commands.keys.length} application (/) commands.`);
        }
        catch (error) {
            console.error(error);
        }
